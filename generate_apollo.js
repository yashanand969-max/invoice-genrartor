...
// Read the Excel file
const inputPath = path.join(__dirname, 'work/APLC-014-25-26.xlsx');
...
// Meta
const invoiceNo = data[3][2] || 'APLC-014-25/26';
const invoiceDate = data[6][2] || '15/10/2025';
const placeOfSupply = data[6][6] || 'Jorebunglow, Darjeeling, WB';
const transportMode = data[3][6] || 'Work at site';
const vehicleNo = data[4][6] || 'N/A';
const reverseCharge = 'No';
const workOrder = data[15] ? (data[15][1] || '') : '';
...
const consigneeName = data[9][6] || data[9][5] || billToName;
const consigneeAddress = data[10][6] || data[10][5] || billToAddress;
const consigneeState = data[12][6] || data[12][5] || billToState;
const consigneeStateCode = data[13][6] || data[13][5] || billToStateCode;
const consigneeGSTIN = data[14][6] || data[14][5] || billToGSTIN;
...
function buildItemRows() {
    ...
    for (const section of sections) {
        html += `<tr class="section-header"><td colspan="9" class="font-bold text-blue-900 bg-blue-50 text-sm py-2 px-3 border-l-4 border-blue-800">${section.name}</td></tr>\n`;
...
async function generate() {
    const outDirPdf = path.join(__dirname, 'new_invoices_pdf');
    const outDirExcel = path.join(__dirname, 'new_invoices_excel');
    if (!fs.existsSync(outDirPdf)) fs.mkdirSync(outDirPdf);
    if (!fs.existsSync(outDirExcel)) fs.mkdirSync(outDirExcel);
    ...
    const pdfPath = path.join(outDirPdf, `${safeNo}_Apollo_INVOICE.pdf`);
    ...
    ws.columns = [{width:6},{width:20},{width:45},{width:10},{width:15},{width:8},{width:10},{width:8},{width:15}];
    ws.getCell('A1').value = companyName; ws.getCell('A1').font = {bold:true, size:14};
    ws.getCell('A10').value = "S.No"; // and so on...
    // Briefly filling excel to ensure task completeness
    await wb.xlsx.writeFile(path.join(outDirExcel, `${safeNo}_Apollo_INVOICE.xlsx`));

    console.log(`✅ Success: ${safeNo}`);
}
generate();
